...
A more subtle source of false positives stems from *temporal valuation errors* due to fluctuations in token prices. MFTracer's transaction-level fund flow analysis relies on historical USD-denominated token prices to infer monetary value at the moment of each transaction.
